...
```go title="Pitfall #2: Not testing" subtitle="Let's take our http requests metric."
...
```go title="Pitfall #2: Not testing" subtitle="Let's consider example impl of LB in ReverseProxy"
...
```go title="Pitfall #2: Not testing" subtitle="Let's MAKE a bug in recording status!"
...
```go title="Pitfall #2: Not testing" subtitle="Let's see if our unit test will catch that!"
...
```diff 52,53,54,55,56,57,58,59 title="Pitfall #2: Not testing" subtitle="Let's see if our unit test will catch that!"
```

```diff 62,63,64,65,66,67,68,69,70 title="Pitfall #2: Not testing" subtitle="Let's see if our unit test will catch that!"
```

```go 35,36,39,58,59,60,61,62,63,64,65,66,67,68,69,70,71 title="Pitfall #2: Not testing" subtitle="Nope as we don't assert any metric value."
...
```go title="Pitfall #2: Not testing - Let's add tests!" subtitle="Prometheus library allows asserting on values!"
...
```go title="Pitfall #2: Not testing - Let's add tests!" subtitle="Let's create metrics instance and initial assertion."
...
```go title="Pitfall #2: Not testing - Let's add tests!" subtitle="Let's add expectations for each case about metric values."
...
```go title="Pitfall #2: Not testing - Let's add tests!" subtitle="And assert on them."
...
```go 99,100,101,102,103,104,105,106,107,108 title="Pitfall #2: Not testing - Let's add tests!" subtitle="Now, test will catch our bug!"
...
```go title="Pitfall #5: Cardinality - Unbounded labels" subtitle="Let's define a metric." 7:14
...
// TODO: Put path, request id in a label

```

```go subtitle="Let's check out our metrics... 😲"

// TODO: Update this examples
...
http_requests_total{code="", method="", request_id="", path="<nil>"} 1
http_requests_total{code="", method="", request_id="", path="lookup example.com on localhost: err..."} 1
http_requests_total{code="", method="", request_id="", path="lookup thanos.io on 8.8.8.8: err..."} 1
http_requests_total{code="", method="", request_id="", path="lookup redhat.com on localhost: err..."} 1
http_requests_total{code="", method="", request_id="", path="syscall: unimplemented EpollWait"} 1
http_requests_total{code="", method="", request_id="", path="syscall.SIGINT: series of unfortunate things happened"} 1
http_requests_total{code="", method="", request_id="", path="unix: test returned fd in TestEpoll"} 1
http_requests_total{code="", method="", request_id="", path="Invalid value for argument: client: nil"} 1
```
