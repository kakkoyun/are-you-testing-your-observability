...
for microservices running on Kubernetes. They collect, store and allow using metrics
and while I think it's quite fascinating how it's done, today we will look closer on the client side of the metric
system. We will focus on how to properly instrument, so add more signals to the application in order to increase observability
, help us understand better the health of production applications, services we are responsible for.
...
Registry stores all registered metrics that will be exposed to Prometheus thanks tp
...
So we run out test, we run 3 request, we assert that response is 502, so Bad Gateway. This is what we
...
1. The four golden signals from SRE book, USE method, RED method.
	...
	- They help to reuse common alerts, recording rules and dashboards. For example, monitoring-mixin project.
	If you are following the same method.
...
1. Let's use the same metric from previous examples, the one that tracks HTTP requests.
...
1. Let's use our usual suspect again.
...
By default they expose 10 buckets, so 12 counters.
...
1. Again let's start with adding our good old http request duration metric with a default set of labels.
...
1. We still don't have enough granularity, let's add more to cover our latencies.

1. Now, it looks better. Let our system run.
...
They can be used to produce arbitrary quantile estimations and the mean.
...
1. Again let's start with adding our good old http request duration metric.
...
1. So coming up with a good bucket layout is hard.

	You need to **know your distribution**.
...
For example, set the highest bucket to whatever value that violates your SLAs and
don't care requests that take longer than that.
